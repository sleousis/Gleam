...
public class RunLogTests
{
    ...
    public async Task Appends_json_lines_and_reads_them_back_skipping_corrupt_lines()
    {
        ...
        var log = new JsonLinesRunLog(storage, "gleam-log.jsonl");
        ...
        var lines = storage.Files["gleam-log.jsonl"].Split('\n', StringSplitOptions.RemoveEmptyEntries);
        Assert.Equal(2, lines.Length);
        Assert.Contains("\"VendorSell\"", lines[0]);

        storage.Files["gleam-log.jsonl"] += "this is not json\n";
        var fresh = new JsonLinesRunLog(storage, "gleam-log.jsonl");
